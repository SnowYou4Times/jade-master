...
public class ContractNetVente extends  ContractNetResponder
{
	...
	protected ACLMessage handleAcceptProposal(ACLMessage cfp, ACLMessage propose,ACLMessage accept) throws FailureException 
	{
		...
		if(liste!=null)
		{
			...
			for (Journey j : liste) {
				var journeyFromStart = catalog.getJourneysFrom(j.getStart());
				var reducedStream = journeyFromStart.stream().filter(oj-> Objects.equals(j.getStop(),oj.getStop())
				&& Objects.equals(j.getDepartureDate(), oj.getDepartureDate()));
				var optionalJourney = reducedStream.findFirst();
				optionalJourney.ifPresent(journeyFound->journeyFound.setPlaces(journeyFound.getNbPlace()-1));
			}
		}
		return inform;
	}
