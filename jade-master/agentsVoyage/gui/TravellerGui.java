...
public class TravellerGui extends JFrame {
	...
	private MyNumberField jNumberField;
	...
	public TravellerGui(TravellerAgent a) {
		...
		betButton.addActionListener(event -> {
			try {
				price = (Integer) jNumberField.getIntValue();
				GuiEvent guiEv = new GuiEvent(this, TravellerAgent.BET_TICKET);
				guiEv.addParameter(price);
				myAgent.postGuiEvent(guiEv);
			} catch(Exception e) {
				JOptionPane.showMessageDialog(TravellerGui.this, "Invalid values. " + e.getMessage(), "Error",
						JOptionPane.ERROR_MESSAGE);
		...
		jNumberField = new MyNumberField();
		jNumberField.setIntValue(0);
...
class MyNumberField extends JTextField
{
